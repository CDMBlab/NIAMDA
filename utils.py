...
def load_data(directory, random_seed):
    ...
    # 读多分类的基因疾病关联数据
    multi_md_associations1 = pd.read_csv(directory + '/wfy_multi_all_mirna_disease_pairs_without_negative.csv',
                                         names=['miRNA', 'disease',
                                                'label'])  # mirna-disease关联 dataFrame【189585，3】正样本5430
    # multi_md_associations1['label'] = multi_md_associations1['label']
    # 读全部的基因疾病关联数据（负样本用0表示，正样本用1表示）
    all_associations = pd.read_csv(directory + '/all_mirna_disease_pairs.csv',
                                   names=['miRNA', 'disease', 'label'])  # mirna-disease关联 dataFrame【189585，3】正样本5430
    ...
    known_associations = all_associations.loc[
        all_associations['label'] != 0]  # df.loc按照条件行取，从189585得5430 ,由于可能做多分类，所以md关联不能用==1了
    ...
    random_negative = unknown_associations.sample(n=known_associations.shape[0], random_state=random_seed,
                                                  axis=0)  # df.sample 按照行随机采样与正样本等量的负样本，axis=0表示按行取
    ...
    sample_df.reset_index(drop=True,
                          inplace=True)  # df.reset_index 打乱m-d样本，drop=True代表需要将重置的索引作为新列插入到dataframe当中，插入也是插入到index当中，不影响数据
    # wfy
...
def build_graph(directory, random_seed):
    ...
    d_sim[: ID.shape[0], :] = torch.from_numpy(
        ID.astype('float32'))  # 给不同类型的节点添加不同特征时，按照节点需要填充tensor矩阵，其余部分用0填充，根据g.ndata['']内的名字来进行划分
    ...
    # 过滤出 samples 中第三列为 1 的行，即有实际关联的边
    positive_samples = samples[samples[:, 2] == 1]
    # 将这些正样本的疾病和 miRNA ID 转换为图中对应的节点索引
    train_disease = [disease_ids_invmap[id_] for id_ in positive_samples[:, 1]]  # 转换疾病节点ID
    train_mirna = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in positive_samples[:, 0]]  # 转换miRNA节点ID
    ...
    multi_mirna_vertices = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in
                            multi_md_associations[:, 0]]  # 从m-d中取m，并将下标转为383-877

    # 过滤出 samples 中第三列为 1 的行，即有实际关联的边
    samples_c = multi_md_associations[multi_md_associations[:, 2] == 1]
    # 将这些正样本的疾病和 miRNA ID 转换为图中对应的节点索引
    train_disease_c = [disease_ids_invmap[id_] for id_ in samples_c[:, 1]]  # 转换疾病节点ID
    train_mirna_c = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in samples_c[:, 0]]  # 转换miRNA节点ID

    # 过滤出 samples 中第三列为 2 的行，即有实际关联的边
    samples_e = multi_md_associations[multi_md_associations[:, 2] == 2]
    # 将这些正样本的疾病和 miRNA ID 转换为图中对应的节点索引
    train_disease_e = [disease_ids_invmap[id_] for id_ in samples_e[:, 1]]  # 转换疾病节点ID
    train_mirna_e = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in samples_e[:, 0]]  # 转换miRNA节点ID

    # 过滤出 samples 中第三列为 3 的行，即有实际关联的边
    samples_t = multi_md_associations[multi_md_associations[:, 2] == 3]
    # 将这些正样本的疾病和 miRNA ID 转换为图中对应的节点索引
    train_disease_t = [disease_ids_invmap[id_] for id_ in samples_t[:, 1]]  # 转换疾病节点ID
    train_mirna_t = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in samples_t[:, 0]]  # 转换miRNA节点ID

    # 过滤出 samples 中第三列为 4 的行，即有实际关联的边
    samples_g = multi_md_associations[multi_md_associations[:, 2] == 4]
    # 将这些正样本的疾病和 miRNA ID 转换为图中对应的节点索引
    train_disease_g = [disease_ids_invmap[id_] for id_ in samples_g[:, 1]]  # 转换疾病节点ID
    train_mirna_g = [mirna_ids_invmap[id_] + ID.shape[0] for id_ in samples_g[:, 0]]  # 转换miRNA节点ID
    ...
    g0.add_edges(multi_disease_vertices, multi_mirna_vertices,
                 data={'multi_label': torch.from_numpy(
                     multi_md_associations[:, 2].astype('float32'))})  # 标签和特征都需要是tensor格式的
    ...
    return (g, g0, sample_disease_vertices, sample_mirna_vertices, ID, IM, samples, train_disease, train_mirna,
            train_disease_c, train_mirna_c, train_disease_e, train_mirna_e, train_disease_t, train_mirna_t,
            train_disease_g, train_mirna_g)
