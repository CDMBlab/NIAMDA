...
### wfy
def Train_multi(directory, epochs, attn_size, attn_heads, out_dim, dim, GCNLayer, beta, dropout, device, lr, wd,
                random_seed, cuda, model_type):
    # 设置python\numpy\torch包的随机种子
    ...
    (g, g0, disease_vertices, mirna_vertices, ID, IM, samples, train_disease, train_mirna, train_disease_c,
     train_mirna_c, train_disease_e, train_mirna_e, train_disease_t, train_mirna_t,
     train_disease_g, train_mirna_g) = build_graph(directory, random_seed)
    ...
    for train_idx, test_idx in kf.split(samples[:, 2]):  # 填入label numpy,返回五次划分结果的索引下标
        ...
        g0.edges[mirna_vertices, disease_vertices].data.update(
            edge_data)  # g0.edges[mirna_vertices, disease_vertices].data
        # g0.edata.update(edge_data) #  Expect number of features to match number of edges. Got 10860 and 25078
        # instead. g0中有25078条边，不能用此方法来更新

        # 由于图的边的添加训练的原因，g0和g边的标记相同，然后分别根据训练边构建子图
        train_eid = g.filter_edges(
            lambda edges: edges.data['train'])  # 根据给的边，获取训练边的index，https://www.cnblogs.com/kaishirenshi/p/8611358.html
        ...
        label_train = g_train.edata['label'].unsqueeze(
            1)  # g.edata['']相当于边数据一个字典，通过'key'查边中含有的数据；等价于 g.edges[index].data['']获取某条边中的数据，利用字典；
        ...
        if model_type == 'HGANMDA':  # in model1,传入子图g_train0
            model = HGANMDA_multi(G=g_train0,
                                  ...
                                  train_mirna=train_mirna,
                                  train_disease_c=train_disease_c,
                                  train_mirna_c=train_mirna_c,
                                  train_disease_e=train_disease_e,
                                  train_mirna_e=train_mirna_e,
                                  train_disease_t=train_disease_t,
                                  train_mirna_t=train_mirna_t,
                                  train_disease_g=train_disease_g,
                                  train_mirna_g=train_mirna_g,
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.set_detect_anomaly(True):  # 设置autograd引擎异常检测的上下文管理器。
                score_train = model(g_train0, g0, train_mirna, train_disease, train_mirna_c, train_disease_c,
                                    train_mirna_e, train_disease_e,
                                    train_mirna_t, train_disease_t, train_mirna_g, train_disease_g, src_train,
                                    dst_train)
            ...
            with torch.no_grad():
                score_val = model(g, g0, train_mirna, train_disease, train_mirna_c, train_disease_c,
                                  train_mirna_e, train_disease_e,
                                  train_mirna_t, train_disease_t, train_mirna_g, train_disease_g, src_test, dst_test)
        ...
        model.eval()
        with torch.no_grad():
            score_test = model(g, g0, train_mirna, train_disease, train_mirna_c, train_disease_c,
                                  train_mirna_e, train_disease_e,
                                  train_mirna_t, train_disease_t, train_mirna_g, train_disease_g, src_test, dst_test)
        ...
        # 计算平均ROC曲线
        mean_fpr = np.linspace(0, 1, 100)  # 生成100个点，作为FPR的x轴
        mean_tpr = np.zeros_like(mean_fpr)

        # 对每一折的ROC曲线进行插值并计算平均TPR
        ...
        # 手动指定每一折的颜色，按照您的要求设置为浅黄、浅蓝、浅绿、浅粉、浅红
        colors = ['#FFEB3B', '#03A9F4', '#8BC34A', '#E91E63', '#F44336']  # 浅黄、浅蓝、浅绿、浅粉、浅红
        linestyle = '--'  # 设置为虚线

        for i, (fpr, tpr, auc_value) in enumerate(zip(fprs, tprs, auc_result)):
            plt.plot(fpr, tpr, color=colors[i], linestyle=linestyle, label=f'Fold {i + 1} (AUC = {auc_value:.4f})')

        # 绘制平均ROC曲线
        plt.plot(mean_fpr, mean_tpr, color='darkviolet', label=f'Mean ROC curve (AUC = {mean_auc:.4f})', linewidth=2)
